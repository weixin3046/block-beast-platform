...
账号名为 3–32 位英文、数字、下划线或连字符；登录密码不能为空且不能全为空白，不设长度上下限，仍受 HTTP 请求体大小限制。生产与开发环境规则一致。昵称可省略，默认使用“用户”加公开 ID。可选头像 `avatar_url` 必须是当前操作人已确认上传的图片 storage_key。
...
用户查询：`GET /v1/admin/users?user_type=real` 或 `user_type=virtual`，省略查全部玩家；接口不返回 admin/operator 后台账号；q 可按公开 ID、登录名、昵称搜索。删除玩家账号使用 `DELETE /v1/admin/users/{userID}`，请求体为 `{"second_password":"后台二级操作密码"}`；该操作保留投注、钱包、账本、聊天和审计历史，仅禁用账号、删除密码凭证并撤销会话，原账号不能再次登录。后台账号不能删除。
...
批量创建且昵称留空时，使用成熟的资料生成库随机生成 2–8 位中文昵称，并排除全部已有虚拟账号昵称；自定义昵称保持原值。发生大量同名碰撞时，会使用 8 位中文编码标识。0093 起虚拟账号创建时在同一事务内生成上分、下分客服房间，并补齐历史虚拟玩家缺失房间，保留已有房间和消息。房间创建不改变虚拟账号的资金权限。
