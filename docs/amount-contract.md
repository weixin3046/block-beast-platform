...
哈希配置及菜单倍率亦直接展示：guess_rate、dodge_rate、road_rate返回实际倍率字符串，保存接受数字或十进制字符串，由后端精确转换为内部整数分数。不再对外使用这些配置的multiplier/divisor字段；投注订单原有payout_rate及赔率快照字段不在本次变更范围。此为表现层变更，不修改数据库已保存的投注限额。

请求金额支持 JSON 数字或十进制字符串：`100`、`1.5`、`"1.5"`。返回金额统一为十进制字符串，并去掉小数末尾多余的零，例如 `"2223.000000"` 返回 `"2223"`、`"1.500"` 返回 `"1.5"`、零返回 `"0"`；`"0.000001"` 保持不变。HTTP与Socket规则一致，只改变展示格式，不改变数据库精度、输入限制或计算结果；decimals仍表示币种支持的最大小数位，不表示响应必须补足的位数。响应不再同时返回 `*_minor`。旧请求传 `stake_minor` 等字段会返回400，不能把旧字段改成小数继续发送。
