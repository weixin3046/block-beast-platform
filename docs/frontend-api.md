...
`count` 省略时创建一个账号；填写 1–100 时以 `login_name` 为前缀生成可排序账号，例如上述请求生成 `sadewf1` 到 `sadewf99`。整批账号使用同一个密码；昵称留空时，随机混用中文人名、日常昵称、英文昵称和游戏 ID，并排除已有虚拟账号昵称。账号 ID 由后端生成，保存响应的 `user_id`。初始余额传真实金额，不乘精度；不需要的币种省略，不传零。创建默认不开启挂机。账号有 player 角色，可使用 `POST /v1/auth/login` 正常登录；不需要先登录玩家账号才能保存挂机配置。创建接口没有请求幂等键，网络超时应先按登录名查询确认，避免盲目重试。
