...
玩家、后台登录及注册默认签发永久访问令牌；刷新接口同样签发永久令牌。
`expires_in: 0` 表示不按时间过期，JWT 不包含 `exp`；客户端不得据此立即刷新或退出。
刷新令牌对应会话也不按时间过期，调用刷新接口仍会轮换刷新令牌，旧刷新令牌不可重用。
退出登录、账号封禁、改密、权限变更和重新登录仍会撤销会话；HTTP 返回 401，
已连接 WebSocket 在后续会话检查时关闭（每秒检查一次，数据库检查有超时）。
用户重新登录后旧会话失效，仍维持单账号单会话。旧 token 保留原到期时间，
重新登录或成功刷新后获得永久 token；已过期或已撤销的 token 不会恢复。
`AUTH_PERMANENT_TOKENS=false` 可使后续新签发令牌恢复原 TTL 模式，不会追溯撤销既有令牌。
...
后台投注列表 `GET /v1/admin/bets?limit=50&offset=0` 返回分页对象 `{ "items": [], "total": 0, "limit": 50, "offset": 0, "has_more": false, "next_offset": null }`，不再直接返回数组。total是符合当前全部筛选条件的订单总数，不受分页参数影响，空页仍返回总数。limit默认50，最大200（超出范围使用50）；offset默认0。前端用 `Math.ceil(total / limit)` 计算总页数，也可用next_offset请求下一页。计数与列表为两次查询，实时新增订单可能使两者短暂不同；按创建时间、订单ID倒序，期间新增数据可能使offset分页发生位移。

后端可配置 LuluAll 补期数据源；经保护性校验补齐的结果通过同一接口返回，
前端无需改动，也不直接访问采集器。实时来源与历史补期均以平台落库结果为准。
...
`GET /v1/lulu/menus` 和 `GET /v1/admin/lulu/prime-time-configs` 自动包含新增的已启用非体力币种。未配置币种的 `payout_multiplier`、`payout_divisor`、`min_stake`、`max_stake` 均为0（金额响应为字符串），八点档 `enabled=false`。零值为未配置占位，不开放投注。保存时可携带全零项（八点档必须关闭），不会写入可投注配置；部分为零的无效赔率/限额仍拒绝。现有非零配置保持原值。八点档尚无配置时也返回房间/单双及躲避的默认项。
...
- 玩家端无需新增接口：进入游戏页时按常规调用 `GET /v1/lulu/menus`，服务端投注校验始终以成交时刻生效配置（八点档或日常）为准；前端在八点档应仅展示单双和躲避，并将单双说明为“猜淘汰房间数量的单双”。
...
新版API接受哈希投注时保存房间级返水配置和代理链快照，之后修改配置、代理等级不影响已接受投注。上下路、竞猜以投注本金为返水基数；哈希躲避仅以正净赢（实际派奖减本金）为基数，亏损或持平不返；每位上级的级差金额按币种最小单位向下取整。模拟、取消、退款单不发放。配置停用或缺失不回退旧佣金；历史版本1订单仍走旧机制，新版本2订单不会叠加旧佣金。
...
真实玩家注册或后台创建真实玩家时，在同一事务内创建上分deposit、下分withdrawal两个客服房间并添加玩家owner成员。失败时整个账号创建事务回滚。0093起虚拟账号也在创建事务内建房，并补齐历史虚拟玩家缺失房间；历史真实player用户由0066补建缺失房间，保留原房间ID、成员及消息。后台空房间不代表存在待处理请求，应根据消息/未读状态区分。
...
后台矩阵包含币种表中除 `category=stamina`（体力）之外的全部币种；尚未配置的币种，其三个倍率和四个投注限额均返回 `"0"`。保存允许这七项全部为零，表示未配置/关闭该币种投注；部分为零仍拒绝。零配置不生成可下注的房间币种记录，玩家菜单不展示该占位配置。已有配置保持原值。
...
6. `GET /v1/wallets/{accountID}/all` 返回全部已启用币种及用户已有钱包；已启用币种尚无钱包记录时，可用余额和冻结余额均返回零（按币种精度格式化），查询不会创建钱包。包含 `decimals`、`available`、`frozen`，可直接显示后端金额字符串；包括用户持有的禁用币种，不要因目录隐藏而丢弃已有资产。
...
哈希躲避返水对齐参考 xMode=true：投注100、实际派奖108，仅按8计算级差返水；亏损或持平不发放。房间级配置和下单等级/比例快照不变。无需新增数据库迁移，升级后由新版Worker结算的版本2订单（包括升级时未结算订单）使用此基数，已结算和已发返水不重算。发布时停止旧Worker后替换，避免新旧算法混跑；版本1旧佣金机制不变。
...
每条返佣还包含 `stake`（来源投注原始投入）、`rebate_base`（本笔返水实际计算基数）和 `rebate_rate_basis_points`（返水比例；100等于1%，例如 80 等于0.8%）。新版级差返水使用结算时保存的历史比例与基数；旧返佣没有保存比例快照，接口按实际返佣金额与投注金额反算，最小金额取整时会有微小差异。金额字段与 `amount` 使用相同币种精度。
...
两个接口from/to均为可选RFC3339带时区时间，开始包含、结束不包含；格式错误或结束不晚于开始返回400。不传不限制时间。返佣时间依赖已有0082迁移，本次更新API即可，无新增迁移。
...
响应仍为数组，保留 `agent_id`、`login_name`、`display_name` 等收款人字段，新增：

- `source_user_id`：产生收益的来源投注用户公开数字 ID。
- `source_login_name` / `source_display_name`：该投注用户当前账号、昵称（非历史快照）。
- `created_at`：收益产生时间，不是投注时间或撤销时间；无法恢复的历史记录为 `null`。

结果按收益时间倒序、ID 倒序，未知时间排最后；保留最多 50 条的现有限制。传任一时间条件时，未知时间记录不参与结果。部署须先执行 `0082_commission_created_at.sql`，再更新 API；新记录自动保存事务时间，旧记录只从精确关联的返佣入账账本回填，不更改资金或账本。个人返佣接口不变。

0081 账号更新：公开ID和邀请码统一为10001–99999的五位数，历史用户按旧ID顺序重新编号。旧六位ID示例不再是有效账号，前端重新读取实际ID。后台创建虚拟账号的 password 仅要求非空且不能全空白，不限制长度（请求体大小限制仍生效）；例如 hellohello 可用，initial_balances 中的字符串 "1" 表示实际数量1。真实账号密码规则不变。`GET /v1/admin/commissions` 新增 login_name/display_name，均为 agent_id 对应的收款用户账号及昵称，不是来源投注玩家。

轮次规则（0080）：每个玩法只允许最近的未完成轮次开放投注，未来预创建轮次状态为 `scheduled`（待开放）。前一期结算或取消后才开放下一期，封盘到结算完成期间不可投注下一期。普通下注和机器人共用校验，直接传未来轮次 ID 同样拒绝；`GET /v1/rounds` 仅返回可投注轮次。机器人选择最小开放期号，skip_min/skip_max 仍表示期数间隔；首次启动先排期。历史未来期已接受的投注保留并正常结算。上线须先停止旧 API/Worker，再运行部署脚本执行 0080 并启动新版服务。
