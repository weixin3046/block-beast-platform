...
| `GET /v1/chat/rooms/{room_id}/messages` | 查询有权限访问的聊天消息；发送使用 WebSocket `chat.send`。 |
| `DELETE /v1/chat/rooms/{room_id}/messages/{message_id}` | 幂等软删除消息，并发送实时删除通知。 |
...
| `GET /v1/external-draws/{game}/history?count=100` | 查询 Worker 从 Lulu 官方实时接口已确认并落库的星海逃杀、怒翎破阵或绿茵疾冲开奖历史。仅已登录用户。 |
