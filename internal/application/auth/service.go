...
type UserRegistrar interface {
	RegisterPasswordUser(ctx context.Context, loginName string, displayName string, passwordHash string, roleCode string, invitationCode int64) (string, error)
}
...
func (service *Service) Register(ctx context.Context, loginName string, displayName string, password string, invitationCode string) (LoginResult, error) {
	...
	userID, err := service.registrar.RegisterPasswordUser(ctx, loginName, displayName, hash, identity.RolePlayer, invite)
	if err != nil {
		return LoginResult{}, err
	}
