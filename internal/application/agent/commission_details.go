...
func (s *Service) ListCommissionDetails(ctx context.Context, owner, currency string, from, to time.Time, limit int) ([]CommissionDetail, error) {
	...
	for rows.Next() {
		...
		if err = rows.Scan(&v.ID, &v.BetID, &v.AgentID, &v.Currency, &v.AmountMinor, &v.Status, &v.GameType, &v.GameName, &v.Sequence, &v.CreatedAt, &v.PlayMode, &v.Selection); err != nil {
			return nil, err
		}
