...
func formatCurrencyStatistic(v *CurrencyStatistic) error {
	...
	for _, pair := range []struct {
		...
		{v.BetLossMinor, &v.BetLoss}, {v.StakeMinor, &v.Stake}, {v.PayoutMinor, &v.Payout}, {v.DepositMinor, &v.Deposit}, {v.TotalDepositMinor, &v.TotalDeposit}, {v.CreditMinor, &v.Credit}, {v.RebateMinor, &v.Rebate}, {v.ClearanceMinor, &v.Clearance}, {v.GiftMinor, &v.Gift}, {v.PenaltyMinor, &v.Penalty}, {v.BalanceMinor, &v.Balance},
	} {
...
func (s *Service) dashboardFunds(ctx context.Context, result *Dashboard, from, to time.Time) error {
 ...
 COALESCE(sum(l.deposit),0),COALESCE(sum(l.deposit),0) + COALESCE(sum(l.credit),0),COALESCE(sum(l.credit),0),COALESCE(sum(l.rebate),0),COALESCE(sum(l.clearance),0),COALESCE(sum(l.gift),0),COALESCE(sum(l.penalty),0),
 ...
 ORDER BY w.currency,COALESCE(u.public_id,0)`, from, to, ids)
	...
	for rows.Next() {
		...
		if err = rows.Scan(&id, &v.Currency, &v.Decimals, &v.BetCount, &v.StakeMinor, &v.PayoutMinor, &v.BetLossMinor, &v.DepositMinor, &v.TotalDepositMinor, &v.CreditMinor, &v.RebateMinor, &v.ClearanceMinor, &v.GiftMinor, &v.PenaltyMinor, &v.BalanceMinor); err != nil {
			return err
		}
