...
func main() {
	...
	defer pool.Close()
	server := &http.Server{Addr: cfg.APIAddress, Handler: httpapi.New(cfg, logger, betting.NewService(pool), pool, wallet.NewPostgresRepository(pool), game.NewPostgresRepository(pool)).Handler()} // 创建HTTP服务器实例
