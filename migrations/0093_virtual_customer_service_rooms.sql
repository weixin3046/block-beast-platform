...
CREATE OR REPLACE FUNCTION ensure_user_customer_service_rooms(player_id UUID)
RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM users u JOIN user_roles ur ON ur.user_id=u.id
        JOIN roles r ON r.id=ur.role_id
        WHERE u.id=player_id AND r.code='player') THEN
        RETURN;
    END IF;
...
-- Backfill virtual players, preserving existing rooms and membership.
SELECT ensure_user_customer_service_rooms(u.id)
FROM users u WHERE u.is_virtual AND EXISTS (
